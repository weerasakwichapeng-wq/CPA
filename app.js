...
/* ทิศทางที่เอกสารแต่ละชนิด "ควรจะเป็น" บนกระดาษ — ตัดสินจากชนิดเอกสารที่เดียวตรงนี้ที่เดียว
   ...
   - ใบชั่งน้ำหนักรถที่โรงงาน → แนวนอน (ตัวใบชั่งเป็นกระดาษแนวนอน ถ่ายมาแล้วตัวอักษรวางตามแนวยาว)
...
function wantedPrintOrientation(img) {
  if (img.closest(".lps-cert-photo") || img.closest("#lpsReceiptPhoto")) {
...
/* เช็คว่าล็อตแนบเอกสารครบหรือยัง — ใบรับรอง FSC ของรถ + ใบเสร็จรับเงิน + รูปใบปิดรถ (POP)
   ...
   (ไม่เช็คข้อมูลอื่น เช่น ล็อตวัตถุดิบ/DRC/รูปตาชั่ง-ยาง ตามที่ตกลงไว้)
...
function renderLotForm(params) {
  ...
  function renderSources() {
    ...
    sources.forEach((g, gi) => {
      // แต่ละแปลงมีปุ่ม ✕ ของตัวเอง — เดิมเป็นข้อความรวมบรรทัดเดียว ถ้าเผลอเพิ่มแปลงผิดมา
      // ต้องลบทั้ง FMU ทิ้งแล้วกรอกรอบชั่งใหม่หมด ทั้งที่ตั้งใจจะเอาออกแค่แปลงเดียว
      const plotChips = el("div", { class: "src-plot-chips" },
        ...(g.plots || []).map(plotName => el("span", { class: "src-plot-chip" },
          plotName,
          el("button", {
            type: "button", class: "src-plot-chip-x",
            title: `เอา ${plotName} ออกจากกลุ่มนี้`,
            onclick: () => {
              // กันไม่ให้กลุ่มเหลือศูนย์แปลง — น้ำหนัก/ยอดเงินผูกอยู่กับกลุ่ม ถ้าไม่เหลือแปลงเลย
              // จะกลายเป็นน้ำหนักที่ไม่มีที่มา ตรวจสอบย้อนกลับไม่ได้
              if ((g.plots || []).length <= 1) {
                alert("⚠️ กลุ่มนี้เหลือแปลงเดียว เอาออกไม่ได้\nถ้าต้องการลบทั้งกลุ่มพร้อมรอบชั่ง ให้กดปุ่ม \"🗑️ ลบทั้ง FMU\"");
                return;
              }
              g.plots = g.plots.filter(p => p !== plotName);
              renderSources();
              recalcFooter();
            },
          }, "✕"),
...
function renderLotDetail(params) {
  ...
  [
    ["ล็อตวัตถุดิบ", lot.transferDocNo],
    ["ผู้ซื้อ", lot.buyer],
    ["DRC %", lot.drcPercent + "%"],
  ...
  function renderPrintSheet() {
    ...
    metaTb.append(
      ...
      el("tr", null, metaRow("รูปแบบผลิต", lot.productForm), metaRow("FSC Claim", lot.fscClaim), metaRow("ล็อตวัตถุดิบ", lot.transferDocNo)),
